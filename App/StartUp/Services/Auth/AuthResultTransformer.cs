...
public class AuthorizationResultTransformer(IOptions<ErrorPortalOption> ep, IOptions<AppOption> ap)
    ...
    if (policyAuthorizationResult is { Forbidden: true, AuthorizationFailure: not null })
    {
      ...
      )
      {
        if (httpContext.Items[Constants.ProblemContextKey] is IDomainProblem problem)
        {
          ...
          p.Extensions.Add(
            new KeyValuePair<string, object?>("traceId", httpContext.TraceIdentifier)
          );
