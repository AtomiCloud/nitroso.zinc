...
public class OtelRedisDatabase(IRedisDatabase redis)
{
  ...
  {
    await redis.SubscribeAsync<OtelRedisMessage<T>>(
      channel,
      async m =>
      {
  ...
  )
  {
    ...
    return redis.Database.StreamAdd(
      key,
      nameValueEntry,
      messageId,
      maxLength,
  ...
  public async Task StreamRead<T>(
    RedisKey key,
    RedisValue position,
    Func<T?, Task> handler,
    int? count = null,
  ...
  public async Task StreamReadGroup<T>(
    RedisKey key,
  ...
  {
    var entries = redis.Database.StreamReadGroup(
      key,
      groupName,
      consumerName,
      position,
  ...
  private OtelRedisMessage<T>? FromNameValueEntry<T>(NameValueEntry[] nameValueEntry)
  {
    ...
    if (contextJson == null || messageJson == null)
      return null;

    var context = JsonSerializer.Deserialize<Dictionary<string, string>>(
      contextJson,
      SerializeOptions
    );
