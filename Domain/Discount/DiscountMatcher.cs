...
public class DiscountMatcher(ILogger<DiscountMatcher> logger) : IDiscountMatcher
{
  ...
  {
    logger.LogInformation("Matching: {Type} {@Values} for {@UserId} with {@Roles}", discount.MatchMode, discount.Matches.ToArray(), userId, roles);
  ...
  {
    return matches.All(x => x.Type switch
    {
  ...
  {
    return matches.Any(x => x.Type switch
    {
      DiscountMatchType.UserId => x.Value == userId,
      DiscountMatchType.Role => roles.Contains(x.Value),
      _ => throw new ArgumentOutOfRangeException()
