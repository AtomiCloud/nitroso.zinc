...
public class AuthHelper(IOptionsMonitor<AuthOption> authOption, ILogger<IAuthHelper> logger) : IAuthHelper
{
  ...
  public bool HasAll(ClaimsPrincipal? user, string field, params string[] scopes)
  {
    ...
    if (!r) logger.LogInformation("No matching scopes. Field: {RequireField}  Needed: {@Require}, Token: {@Token}", field, scopes, s);
    return r;
  }
