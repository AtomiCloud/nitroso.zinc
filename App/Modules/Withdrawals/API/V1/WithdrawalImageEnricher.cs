...
public interface IWithdrawalImageEnricher
{
  ...
  Task<Result<IEnumerable<WithdrawalPrincipalRes>>> Enrich(
    IEnumerable<WithdrawalPrincipalRes> booking
  );
...
public class WithdrawalImageEnricher(IWithdrawalStorage storage) : IWithdrawalImageEnricher
{
  ...
  {
    if (booking.Complete?.Receipt == null)
      return booking;

    return await storage
      .Get(booking.Complete.Receipt)
      .Select(link => booking with { Complete = booking.Complete with { Receipt = link } });
  }

  public async Task<Result<IEnumerable<WithdrawalPrincipalRes>>> Enrich(
    IEnumerable<WithdrawalPrincipalRes> booking
  )
  {
