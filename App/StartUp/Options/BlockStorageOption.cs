...
public class BlockStorageEndpoint
{
  [Required, MinLength(1)] public string Host { get; set; } = string.Empty;
  [Required, Range(0, ushort.MaxValue)] public int Port { get; set; } = 9000;

  [Required, AllowedValues("http", "https")] public string Scheme { get; set; } = "http";
}
...
public class BlockStorageOption
{
  ...
  [Required] public BlockStorageEndpoint Write { get; set; } = new();

  [Required] public BlockStorageEndpoint Read { get; set; } = new();
  ...
  [Required] public bool EnsureBucketCreation { get; set; } = false;
