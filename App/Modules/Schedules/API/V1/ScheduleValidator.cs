...
public class ScheduleRangeReqValidator : AbstractValidator<ScheduleRangeReq>
{
  ...
  {
    this.RuleFor(x => x.From).NotNull().DateValid();
    this.RuleFor(x => x.To).NotNull().DateValid();
  }
}
...
public class ScheduleBulkUpdateReqValidator : AbstractValidator<ScheduleBulkUpdateReq>
{
  ...
  {
    this.RuleFor(x => x.Schedules).NotNull();
    this.RuleForEach(x => x.Schedules).NotNull().SetValidator(new SchedulePrincipalReqValidator());
  }
}
...
public class ScheduleRecordReqValidator : AbstractValidator<ScheduleRecordReq>
{
  ...
  {
    this.RuleFor(x => x.Confirmed).NotNull();
    this.RuleFor(x => x.JToWExcluded).NotNull();
    this.RuleForEach(x => x.JToWExcluded).NotNull().TimeValid();

    this.RuleFor(x => x.WToJExcluded).NotNull();
    this.RuleForEach(x => x.WToJExcluded).NotNull().TimeValid();
  }
}
...
public class SchedulePrincipalReqValidator : AbstractValidator<SchedulePrincipalReq>
{
  ...
  {
    this.RuleFor(x => x.Date).NotNull().DateValid();
    this.RuleFor(x => x.Record).NotNull().SetValidator(new ScheduleRecordReqValidator());
  }
}
