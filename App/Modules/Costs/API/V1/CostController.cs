  ...
  IAuthHelper h) : AtomiControllerBase(h)
{
  ...
  {
    var x = await service.History()
      .Then(x => x.Select(a => a.ToRes()), Errors.MapNone);
  ...
  {
    var cost = await
      costReqValidator.ValidateAsyncResult(req, "Invalid CreateCostReq")
  ...
  {
    var cost = await service.GetCurrent()
      .Then(x => x?.ToRes(), Errors.MapNone);

    return this.ReturnNullableResult(cost, new EntityNotFound(
      "Cost Not Found", typeof(CostPrincipal), "none"));
  }
