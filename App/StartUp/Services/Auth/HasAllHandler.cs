...
public class HasAllHandler(
  ILogger<AuthorizationHandler<HasAllRequirement>> logger,
  IHttpContextAccessor httpContextAccessor
) : AuthorizationHandler<HasAllRequirement>
{
  ...
  )
  {
    ...
    if (scopes == null)
    {
      httpContextAccessor.HttpContext?.SetProblem(
        requirement,
        "No scopes found in the token (AND)",
        requirement.Field,
        scopes
      );
    ...
    else
    {
      httpContextAccessor.HttpContext?.SetProblem(
        requirement,
        "Unauthorized, not matching roles or scope (AND)",
        requirement.Field,
        scopes
      );
